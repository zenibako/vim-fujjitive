...
# ── Test: Ancestors and Other mutable sections ───────────────────────────────
...
" Check that 'Ancestors' section exists
let has_ancestors = 0
for l in lines
  if l =~# '^Ancestors'
    let has_ancestors = 1
...
if !has_ancestors
  echoerr 'Missing "Ancestors" section. Buffer: ' . content
  cquit 1
endif

" Check that old 'Current branch' section does NOT exist
for l in lines
  if l =~# '^Current branch '
    echoerr 'Found old "Current branch" section that should have been renamed. Buffer: ' . content
    cquit 1
  endif
endfor
VIMSCRIPT
then pass "Status buffer shows Ancestors section (not Current branch)"
else fail "Status buffer shows Ancestors section (not Current branch)"
fi
